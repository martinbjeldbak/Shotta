...
local function InitializeOptions(_, triggers, screenshotFrame, icon)
	local frame = CreateFrame("Frame")
	local category, layout = Settings.RegisterCanvasLayoutCategory(frame, Shotta.ADDON_NAME)
	...
	-- or a settings list:
	-- local category, layout = Settings.RegisterVerticalLayoutCategory(Shotta.ADDON_NAME)
	...
	-- 	local discordButton = CreateFrame("Button", nil, layout, "GameMenuButtonTemplate")
	-- 	-- discordButton:SetPoint("LEFT", layout, "RIGHT", 85, 40)
	...
	-- 	-- t.helpText:SetText(format(ns.T["saveLocationHelpText"], folderName(), folderName()))
	-- 	--
	-- 	-- -- Love text
	...
	-- 	--
	-- 	-- -- local category, layout = Settings.RegisterCanvasLayoutCategory(frame, "xx", "yy")
